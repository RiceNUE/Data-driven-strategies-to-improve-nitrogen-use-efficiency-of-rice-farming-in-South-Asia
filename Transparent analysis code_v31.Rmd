...
1.4) Reporting sample size for each year and region (Table S8 in Supplementary Information 9)
...
2.2) Analysing variation in mineral macronutrient application (Figure S3 in Supplementary Information 3)
...
## N rate
Nrate <- ggplot(nib6,aes(Region_name, N_kg_ha))+
  geom_boxplot(aes(fill= Region_name)) +
  stat_summary(fun=mean, geom="point", shape=23, size=3, color="black", fill="red") +
  # stat_summary(fun = mean, geom = "text", aes(label = round(..y.., 1)), vjust = -0.5, size = 5, color = "black") +
...
## P rate
Prate <- ggplot(nib6,aes(Region_name,P2O5_ha))+
  geom_boxplot(aes(fill=Region_name)) +
  stat_summary(fun=mean, geom="point", shape=23, size=3, color="black", fill="red") +
  # stat_summary(fun = mean, geom = "text", aes(label = round(..y.., 1)), vjust = -0.5, size = 5, color = "black") +
...
## K rate
Krate <- ggplot(nib6,aes(Region_name,K2O_ha))+
  geom_boxplot(aes(fill=Region_name)) +
  stat_summary(fun=mean, geom="point", shape=23, size=3, color="black", fill="red") +
  # stat_summary(fun = mean, geom = "text", aes(label = round(..y.., 1)), vjust = -0.5, size = 5, color = "black") +
...
# Save figures
setwd('[insert working directory]')
ggsave(filename = "FigureS2a.png", plot = Nrate, width = 10, height = 6, units = "in", dpi = 700)
ggsave(filename = "FigureS2b.png", plot = Prate, width = 10, height = 6, units = "in", dpi = 700)
ggsave(filename = "FigureS2c.png", plot = Krate, width = 10, height = 6, units = "in", dpi = 700)
...
# Input monsoon rice area assumptions defined in Table S6 in supplementary information 5
...
2.4) Comparing nitrogen outcomes across regions (Figure 1b and Table S1)
...
PercBreak <- data.frame(regs$Region)
colnames(PercBreak)[1] <- "Region"
PercBreak$LowInput_HighOutput <- NA
PercBreak$HighInput_HighOutput <- NA
PercBreak$LowInput_LowOutput <- NA
PercBreak$HighInput_LowOutput <- NA

for(i in 1:nrow(PercBreak)){
PercBreak[i,"LowInput_HighOutput"] <- paste0(round((
  length(which(nib6$Region == PercBreak[i,"Region"] &
                 nib6$yield_t_ha >= 5 & nib6$N_kg_ha < 114)) /
  length(which(nib6$Region == PercBreak[i,"Region"])) *
    100
),1),"%")
}
...
for(i in 1:nrow(PercBreak)){
PercBreak[i,"HighInput_HighOutput"] <- paste0(round((
  length(which(nib6$Region == PercBreak[i,"Region"] &
                 nib6$yield_t_ha >= 5 & nib6$N_kg_ha >= 114)) /
  length(which(nib6$Region == PercBreak[i,"Region"])) *
    100
...
for(i in 1:nrow(PercBreak)){
PercBreak[i,"LowInput_LowOutput"] <- paste0(round((
  length(which(nib6$Region == PercBreak[i,"Region"] &
                 nib6$yield_t_ha < 5 & nib6$N_kg_ha < 114)) /
  length(which(nib6$Region == PercBreak[i,"Region"])) *
    100
...
for(i in 1:nrow(PercBreak)){
PercBreak[i,"HighInput_LowOutput"] <- paste0(round((
  length(which(nib6$Region == PercBreak[i,"Region"] &
                 nib6$yield_t_ha < 5 & nib6$N_kg_ha >= 114)) /
  length(which(nib6$Region == PercBreak[i,"Region"])) *
    100
...
PercBreak$Region <- regs$Region_figurename
...
setwd('[insert working directory]')
write.csv(PercBreak, "TableS1.csv")
ggsave(filename = "Figure1b.png", plot = Figure1b, width = 10, height = 6, units = "in", dpi = 400)

```

2.5) Get mean and standard error for each variable in Figure 1a (Table S2 in Supplementary Information 1)
...
2.6) Testing statistical signficance of regional differences for each variable in Figure 1a (Table S3 in Supplementary Information 1)
...
2.7) Clarifying sample size within different combinations of N rate and region (Table S4 in Supplementary Information 3)
...
2.8) Modelling average NUE for any given nitrogen rate in each region (Figure S2 in Supplementary Information 2)
...
2.9) Modelling 'frontier' NUE for any given nitrogen rate in each region (Figure 2 & Figure S1)
...
ggsave(filename = "FigureS1a.png", plot = FigureS1a, width = 10, height = 6, units = "in", dpi = 700)
ggsave(filename = "FigureS1b.png", plot = FigureS1b, width = 10, height = 6, units = "in", dpi = 700)

```

2.10) Figure 2 without addressing overfitting in frontier splines for Odisha and Nepal (Figure S10 in Supplementary Information 10)
...
3.11) Isolating average relationship between nitrogen rate and yield for each region (Figure S4 in Supplementary Information 4)
...
regs$R2_linear <- NA  # Add column to store R² of the linear model
regs$R2_segmented <- NA  # Add column to store R² of the segmented model
...
r2_linear <- summary(lin.model)$r.squared
  
# Calculate R² for segmented model (pseudo R² based on deviance)
r2_segmented <- 1 - sum(residuals^2) / sum((y - mean(y))^2)
  
# Store the R² values in the 'regs' dataframe
regs[i, "R2_linear"] <- r2_linear
regs[i, "R2_segmented"] <- r2_segmented
...
TableS5 <- regs[,c("Region", "R2_linear", "R2_segmented")]
TableS5$Region <- regs$Region_figurename
TableS5$R2_linear <- round(TableS5$R2_linear, 2)
TableS5$R2_segmented <- round(TableS5$R2_segmented, 2)
...
3.13) Visualising potential NUE increases (Figure S11 in Supplementary Information 12)
...
ggplot() +
  ...
  geom_vline(xintercept = 50, linetype = "solid", color = "black", size = 0.5) +
  geom_vline(xintercept = regs[i, "N_rate_breakpoint"], linetype = "dashed", color = "black", size = 0.5) +
  theme_bw() +
  xlim(c(0,400)) +
  ylim(c(0,10000)) +
  labs(x = "N rate (kg/ha)", y = "Predicted yield (kg/ha)") +
  theme(axis.text=element_text(size=10), axis.title=element_text(size=13)) +
  theme(legend.position=c(0.899,0.2045))
)
}
...
# Input price and subsidy assumptions defined in Table S6 Supplementary information 5
...
3.16) Robustness check for Table 1 (Table S10 in Supplementary Information 11)
...
4.11) Visualising predictor importance for random forest models predicing relative NUE for different regions (Figure 3) and different nitrogen application rates (Figure S5 in Supplementary Information 7)
...
## Make table
FigureS5 <- formattable(simples1_region,
            row.names = F,
            col.names = c(
              "Predictor",
              "Category",
...
4.12) Dependence plots for most importance predictors of relative NUE (Figure 4 & Figure S9 in Supplementary Information 8) 
...
4.15) Checking robustness of Figure 3 results across seasons: visualising Shapley values (Figure S6 in Supplementary Information 7)
...
4.20) Checking robustness of Figure 3 results across models: visualising predictor importance for random forest models that predict relative NUE as a continuous variable, instead of a cateogircal variable (Figure S7 in Supplementary Information 7)
...
## Make table
FigureS7 <- formattable(simples3_reordered,
            row.names = F,
            col.names = c(
              "Predictor",
              "Category",
...
4.21) Checking robustness of Figure 3 results across models: Visualising predictor imoprtance for random forest models that predict yield for nitrogen rate 0-200 kg/ha (Figure S8 in Supplementary Information 7)
...
## Make table
FigureS8 <- formattable(simples4_reordered,
            row.names = F,
            col.names = c(
              "Predictor",
              "Category",
...
export_formattable(FigureS8,"FigureS8.png")
